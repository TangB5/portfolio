...
// Interface temporaire pour Motif (si vous ne voulez pas le type dans le fichier séparé)
interface Motif {
    id: number;
    src: string;
    alt: string;
    ...
    origin: string;
    usage: string;
    status: 'available' | 'revision' | 'upcoming';
}
...
export default function Culture() {
    ...
    const motifs: Motif[] = [ // Utilisation du type Motif
        {
            id: 1,
            src: '/motif-wax.jpg',
    ...
    const getStatusDisplay = (status: Motif['status']) => {
        switch (status) {
            case 'available':
                return {
                    text: 'Disponible',
            ...
            default:
                return { text: '', icon: '', color: colors.dark, opacity: 'opacity-100', cursor: 'cursor-default' };
        }
    };
        ...
        <div className="min-h-screen relative" style={{ backgroundColor: colors.dark, color: colors.light }}>
            ...
            <div className="pt-28 pb-16 px-4 text-center relative overflow-hidden">
                ...
                <div className="absolute inset-0 opacity-5 z-0">
                    {/* J'ai commenté Plan car il ne semble pas être importé */}
                    {/* <Plan/> */}
            ...
            >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    ...
                    <span
                        ...
                        style={{ backgroundColor: statusDisplay.color, color: colors.dark }}
                    >
                      <i className={`${statusDisplay.icon} mr-1`}></i>
                        {statusDisplay.text}
                                ...
                                <div className="p-5 flex-1">
                                    <h3 className={`font-bold text-lg mb-2 ${statusDisplay.opacity}`}>{motif.title}</h3>
                                    <p className={`text-sm opacity-80 mb-4 ${statusDisplay.opacity}`}>{motif.description}</p>

                                    <div className="flex justify-between items-center text-xs opacity-70">
                                        <span className={statusDisplay.opacity}>{motif.origin}</span>
                                        <motion.span
                                ...
                                {/* Modal pour les détails du motif (Ouvert uniquement si 'available' et cliqué) */}
